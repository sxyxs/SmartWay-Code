...
class SSTrainer(BaseVLNCETrainer):
    def __init__(self, config=None):
        ...
        self.wp_loss_method = config.IL.wp_loss_method
    ...
    def save_checkpoint(self, epoch: int, step_id: int) -> None:
        torch.save(
            obj={
                ...
                "waypoint_predictor_state": self.waypoint_predictor.state_dict(),
                "config": self.config,
    ...
    def train_ml(self, in_train=True, train_tf=False):
        ...
        for stepk in range(self.max_len):
            ...
            if 'CMA' in self.config.MODEL.policy_name:
                ...
                logits, rnn_states,cand_rgb, cand_depth, cand_direction, cand_mask, candidate_lengths, batch_angles, batch_distances,waypoint_heatmap_logits,vis_text_feats,last_action,batch_img_idxes = self.policy.net(
                    mode = "waypoint",
                    waypoint_predictor = self.waypoint_predictor,
                    observations = batch,
                    in_train = in_train,DINO_processor=self.DINO_processor,
                    masks = not_done_masks,
                    rnn_states = rnn_states,
                    instruction = language_features,
                    text_mask = lang_masks,
                    headings = headings,
                    vis_text_feats = vis_text_feats,
            ...
            if train_tf:
                # Step 1: 使用 oracle 提供的候选索引生成动作，并根据一定概率选择 oracle 动作
                ...
                # 计算当前的 cross-entropy 损失
                current_loss = F.cross_entropy(logits, oracle_actions.squeeze(1), reduction="none")
                
                # Step 2: 筛选未停止的角度及其对应索引
                selected_angles = np.array([
                    batch_angles[i][oracle_cand_idx[i]] 
                    if not oracle_stop[i] else None  # 如果是 stop，则设置为 None
                    for i in range(len(oracle_cand_idx))
                ])
                non_stop_indices = [i for i, angle in enumerate(selected_angles) if angle is not None]       
                selected_angles = np.array([angle for angle in selected_angles if angle is not None])

                # Step 3: 将角度转换为索引 (unmask_id)，并创建 ground truth mask
                unmask_id = ((2 * np.pi - selected_angles) / (2 * np.pi) * 12).astype(int)
                
                # 构建 ground truth mask
                gt_wp_mask = torch.zeros_like(waypoint_heatmap_logits)  # 初始化为 0
                gt_wp_mask[non_stop_indices, unmask_id] = 1
                
                # 提取非停止的 heatmap logits 和 ground truth
                gt_wp = waypoint_heatmap_logits[non_stop_indices] * gt_wp_mask[non_stop_indices]
                waypoint_heatmap_logits = waypoint_heatmap_logits[non_stop_indices]
                assert waypoint_heatmap_logits.size(0) == gt_wp.size(0)
                batch_img_idxes = np.array(batch_img_idxes)
                un_stop_img_index = batch_img_idxes[non_stop_indices]
                
                # Step 4: 反转图片索引并去重
                mapping = {0: 0, 1: 11, 2: 10, 3: 9, 4: 8, 5: 7, 6: 6, 7: 5, 8: 4, 9: 3, 10: 2, 11: 1}
                un_stop_img_index = [list(mapping[x] for x in img_idxes) for img_idxes in un_stop_img_index] # reverse 
                unique_un_stop_img_index = [list(set(x for x in img_idxes)) for img_idxes in un_stop_img_index] # reverse and unique
                
                # 获取 ground truth 索引
                gt_id = []
                for ii,idx in enumerate(np.array(oracle_cand_idx)[non_stop_indices]):
                    ids = un_stop_img_index[ii][idx] 
                    gt_id.append(ids)
                gt_id = torch.tensor(gt_id)
                
                # Step 5: 计算损失
                if len(waypoint_heatmap_logits) != 0:
                    # 根据不同方法计算 waypoint loss
                    if not self.config.ADAPTOR:
                        if self.wp_loss_method == 'orig':
                            gt_wp = gt_wp.flatten(start_dim=1, end_dim=2)
                            waypoint_heatmap_logits = waypoint_heatmap_logits.flatten(start_dim=1, end_dim=2)
                            assert gt_wp.shape == waypoint_heatmap_logits.shape
                            wp_loss = (criterion_mse(waypoint_heatmap_logits, gt_wp).sum()/waypoint_heatmap_logits.size(0))/120
                            ml_loss =ml_loss+ torch.sum(current_loss) +wp_loss
                        elif self.wp_loss_method == 'ave':
                            logits_per_image = waypoint_heatmap_logits.view(waypoint_heatmap_logits.size(0), 12, -1).mean(dim=-1)
                            wp_loss = criterion_ce(logits_per_image,torch.tensor(unmask_id).cuda())
                            ml_loss =ml_loss+ torch.sum(current_loss) +wp_loss*0.1
                        elif self.wp_loss_method == 'max':
                            logits_per_image = waypoint_heatmap_logits.view(waypoint_heatmap_logits.size(0), 12, -1).max(dim=-1).values
                            wp_loss = criterion_ce(logits_per_image,torch.tensor(unmask_id).cuda())
                            ml_loss =ml_loss+ torch.sum(current_loss) +wp_loss*0.1
                        else:
                            ml_loss =ml_loss+ torch.sum(current_loss)
                    else:# ADAPTOR loss
                        # loss应该是先提取logits_per_image, 只考虑orical不是stop的, 然后mask住没考虑的图片,最后candidate
                        logits_per_image = waypoint_heatmap_logits.view(waypoint_heatmap_logits.size(0), 12, -1).max(dim=-1).values
                        loss_mask = torch.zeros_like(logits_per_image).bool()  # 全零布尔 mask
                        cancidate_mask = torch.zeros_like(logits_per_image)

                        # 构造mask
                        for i, valid_indices in enumerate(unique_un_stop_img_index):
                            # 设置有效的indices为1
                            cancidate_mask[i, valid_indices] = 1
                            # 强制GT对应的index为1
                            assert cancidate_mask[i, gt_id[i]] == 1
                        logits_per_image = logits_per_image*cancidate_mask
                        wp_loss = criterion_ce(logits_per_image,gt_id.cuda())
                        ml_loss =ml_loss+ torch.sum(current_loss) +wp_loss
                else:
                    ml_loss =ml_loss+ torch.sum(current_loss)
                # +torch.sum(test_loss)
            ...
            env_actions = []
            non_action_0_ids = []  # 保存不是 action 0 的 ID
            for j in range(logits.size(0)):
                if actions[j].item() == candidate_lengths[j]-1:
                    ...
                    non_action_0_ids.append(j)
            vis_text_feats = vis_text_feats[non_action_0_ids]
        ...
        with TensorboardWriter(
            ...
            for epoch in range(self.start_epoch, self.config.IL.epochs):
                ...
                for batch_idx in t_:
                    ...
                    print(f"Epoch{epoch}, Batch {batch_idx + 1}/{batches_per_epoch} took {batch_duration:.2f} seconds.")
