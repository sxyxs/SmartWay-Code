...
class SSTrainer(BaseVLNCETrainer):
    ...
    def save_checkpoint_adaptor(self, epoch: int, step_id: int) -> None:
        torch.save(
            obj={
                "state_dict": self.policy.state_dict(),
    ...
    def train_ml(self, in_train=True, train_tf=False, train_rl=False):
        ...
        for stepk in range(self.max_len):
            ...
            if 'VLNBERT' in self.config.MODEL.policy_name:
                ...
                # )
                logits, h_t,cand_rgb, cand_depth, cand_direction, cand_mask, candidate_lengths, batch_angles, batch_distances,waypoint_heatmap_logits,batch_img_idxes = self.policy.net(
                    mode = "waypoint",
                    waypoint_predictor = self.waypoint_predictor,
                    observations = batch,
                    in_train = in_train,DINO_processor=DINO_processor,
                    masks = not_done_masks,
                    lang_feats = language_features,
                    lang_masks = lang_masks,
                    headings = headings,vis_text_feats=language_features
                ) # language_features state+instruction
            ...
            if train_rl:
                ...
                selected_angles = np.array([
                    ...
                    if not oracle_stop[i] else None  # 
                ...
                if len(waypoint_heatmap_logits) != 0:
                    ...
                    for ii, valid_indices in enumerate(unique_un_stop_img_index):
                        temp_mask = -1 * torch.ones_like(logits_per_image[ii])
                        temp_mask[valid_indices] = logits_per_image[ii, valid_indices]
                        logits_per_image[ii] = temp_mask
                        assert temp_mask[gt_id[ii].item()] != 1
        ...
        with TensorboardWriter(
            ...
            for epoch in range(self.start_epoch, self.config.IL.epochs):
                ...
                if self.local_rank < 1:  # and epoch % 3 == 0:
                    self.save_checkpoint_adaptor(epoch, self.step_id)
